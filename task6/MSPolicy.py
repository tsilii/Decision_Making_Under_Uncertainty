...
def solve_mssp(state, nodes, t_now, params):
    ...
    if vent_counter > 0:
        ...
        if remaining > 0:
            m.v[0].fix(1)
        if remaining > 1:
            for nid in [n['id'] for n in nodes if n['stage'] == 1]:
                m.v[nid].fix(1)
        if remaining > 2:
            for nid in [n['id'] for n in nodes if n['stage'] == 2]:
                m.v[nid].fix(1)
...
class MultiStageSPPolicy:
    ...
    def __init__(self, L=3, B=None, S_init=500):
        ...
        B:      branching factor — int (uniform) or list per stage, e.g. [20, 4].
                Default [20, 4]: 20 stage-1 nodes + 80 stage-2 nodes = 100 nodes, ~1002 binaries.
