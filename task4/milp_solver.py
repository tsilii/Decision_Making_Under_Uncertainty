...
def solve_milp_subproblem(T_r1_init, T_r2_init, H_init, low_r1_init, low_r2_init,
                          t_start, prices_sub, occ1_sub, occ2_sub, params):
    """
    Solve the MILP for the remaining horizon starting from a given intermediate state.

    t_start   : original timeslot index at which this sub-problem begins (used to
                 look up the correct outdoor temperatures).
    prices_sub, occ1_sub, occ2_sub : arrays of length T_sub (remaining timeslots).
    Returns the optimal total cost for those T_sub timeslots.
    """
    T_sub = len(prices_sub)
    if T_sub == 0:
        return 0.0
    ...
    H_high    = float(params['humidity_threshold'])
    U_vent    = int(params['vent_min_up_time'])
    T_out     = list(params['outdoor_temperature'])[t_start:]
    M_temp    = 100
    M_hum     = 200

    occ = {(1, t): occ1_sub[t] for t in times}
    occ.update({(2, t): occ2_sub[t] for t in times})

    model = ConcreteModel()
    model.T = RangeSet(0, T_sub - 1)
    model.R = Set(initialize=[1, 2])
    ...
    model.obj = Objective(
        expr=sum(prices_sub[t] * (sum(model.p[r, t] for r in R) + P_vent * model.v[t])
                 for t in model.T),
        sense=minimize)

    model.init_T1 = Constraint(expr=model.Temp[1, 0] == T_r1_init)
    model.init_T2 = Constraint(expr=model.Temp[2, 0] == T_r2_init)
    model.init_H  = Constraint(expr=model.H[0] == H_init)

    # Propagate override flags from the incoming state
    if low_r1_init == 1:
        model.init_u1 = Constraint(expr=model.u[1, 0] >= 1)
    if low_r2_init == 1:
        model.init_u2 = Constraint(expr=model.u[2, 0] >= 1)
    ...
    def vent_uptime(model, t):
        horizon  = min(t + U_vent, T_sub)
        duration = min(U_vent, T_sub - t)
        return sum(model.v[tau] for tau in range(t, horizon)) >= duration * model.s[t]
    model.vent_uptime = Constraint(model.T, rule=vent_uptime)
